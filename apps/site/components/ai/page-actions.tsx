...
export function LLMCopyButton({ markdownUrl }: LLMCopyButtonProps) {
  ...
  const [checked, onClick] = useCopyButton(async function copyMarkdown() {
...
export function ViewOptions({ markdownUrl, githubUrl }: ViewOptionsProps) {
  const [q, setQ] = useState(`Read ${markdownUrl}, I want to ask questions about it.`);

  useEffect(
    function syncQ() {
      setQ(`Read ${new URL(markdownUrl, window.location.origin)}, I want to ask questions about it.`);
    },
    [markdownUrl]
  );
  const items = [
    { title: 'Open in GitHub', href: githubUrl, icon: GitHubIcon },
    { title: 'Open in Scira AI', href: `https://scira.ai/?${new URLSearchParams({ q })}`, icon: SciraIcon },
    {
      title: 'Open in ChatGPT',
      href: `https://chatgpt.com/?${new URLSearchParams({ hints: 'search', q })}`,
      icon: ChatGPTIcon
    },
    { title: 'Open in Claude', href: `https://claude.ai/new?${new URLSearchParams({ q })}`, icon: ClaudeIcon },
    {
