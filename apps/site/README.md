...
| Piece | Where | Purpose |
| --- | --- | --- |
| `legacy-tokens.css` import | `app/layout.tsx` | The theme, shared verbatim with Storybook (`apps/docs/.storybook/recipes.ts`) and the browser test projects (`configs/vitest.setup.mts`). Defines the `--ux-*` custom properties every component's CSS reads. Imported **before** `global.css` so its own `body` rule loses the cascade to Fumadocs' and only the custom properties apply site-wide |
