...
feat: bring Drawer and Popover onto the composition model and route every overlay's props the same way
...
- **BREAKING (`Modal`):** `isOpen`, `defaultOpen`, `onOpenChange`, `isKeyboardDismissDisabled`, and
  `shouldCloseOnInteractOutside` are now flat props, so a Modal can be controlled without a
  `ModalTrigger`. `overlayProps` and `containerProps` keep targeting the backdrop and the positioned
  container, but they no longer accept those open-state props, and `containerProps` no longer
  accepts `Flex` layout props.
