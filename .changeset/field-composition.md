...
`Select` keeps its shorthand field props and now takes its options through `options` instead of `children`. `children` composes the full interior with the new `SelectValue` component.
