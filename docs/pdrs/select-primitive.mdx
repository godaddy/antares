...
- `value` — Receives `valueProps` (type: `DOMAttributes` with `id`), `selectedItem`, `selectedItems`. Source: [`useSelect`](https://github.com/adobe/react-spectrum/blob/main/packages/%40react-aria/select/src/useSelect.ts#L215) returns `valueProps: { id: valueId }`. Consumers handle placeholder rendering using the provided selection data.
...
| `value` | Displayed selected value | No | No | `valueProps` (DOMAttributes with `id`), `selectedItem`, `selectedItems` |
