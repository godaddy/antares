...
**Slot structure**:
- `trigger` — Receives `triggerProps` (type: `AriaButtonProps`), `focusProps`, `hoverProps`, `ref`. Source: [`useSelect`](https://github.com/adobe/react-spectrum/blob/main/packages/%40react-aria/select/src/useSelect.ts#L40) returns `triggerProps: AriaButtonProps`. React Aria provides `aria-haspopup="listbox"`, `aria-expanded` (boolean), `aria-controls` via `useOverlayTrigger`.
...
**ARIA Button + Listbox Pattern**:

Select follows React Aria's approach: a button trigger with `aria-haspopup="listbox"` that controls a listbox popup. This pattern is used for non-editable select components (as opposed to the combobox pattern for editable/searchable selects like ComboBox).

| Attribute | Source | Notes |
|-----------|--------|-------|
| `role="button"` | Native or React Aria | Implicit on `<button>` elements, or set by `useButton` for other elements. |
...
| `aria-expanded` | React Aria | Boolean value indicating open state. Provided by `useOverlayTrigger`. Source: [`useOverlayTrigger.ts:58`](https://github.com/adobe/react-spectrum/blob/main/packages/%40react-aria/overlays/src/useOverlayTrigger.ts#L58) |
...
**Why not `role="combobox"`?** The combobox role is for components that combine a text input with a listbox (editable, filterable). React Aria's `useSelect` is for non-editable selects and intentionally uses the button pattern. See [React Aria Select docs](https://react-spectrum.adobe.com/react-aria/useSelect.html) vs [React Aria ComboBox docs](https://react-spectrum.adobe.com/react-aria/useComboBox.html).

**Validation state**: React Aria exposes `isInvalid`, `validationErrors`, and `validationDetails` for the component to use (e.g., showing error messages), but does not set `aria-invalid` on the button trigger. Form validation is handled via the hidden `<select>` element when `validationBehavior="native"`, or exposed via render props when `validationBehavior="aria"`.
...
### 9. No Manual ARIA Overrides

**Pattern**: `Select` does NOT manually add ARIA attributes beyond what React Aria provides.

**Why**: React Aria's `useSelect` and `useOverlayTrigger` provide all necessary ARIA attributes (`aria-haspopup`, `aria-expanded`, `aria-controls`, `aria-labelledby`). The button pattern (not combobox) is correct for non-editable selects.

**Rationale**:
- `role="button"` (implicit or explicit) is correct for the trigger - combobox role is for editable/filterable components
- `aria-required`, `aria-invalid`, `aria-disabled` are not standard on button elements - validation is handled via the hidden `<select>` and exposed via render props
- Aligns with React Aria Components' approach

**Invariant impact**: Fully conforms to Bento invariant #22 ("ARIA Props from React Aria").

**Risk if changed**: Manually adding `role="combobox"` or validation ARIA attributes would misrepresent the component's semantics and deviate from React Aria/RAC patterns.
