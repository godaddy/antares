---
title: SegmentedController
description: A segmented controller is a linear set of two or more segments, each of which functions as a button. A segmented controller allows users to select options, switch views, or sort elements.
---

import { Meta, ArgTypes, Source, Story } from '@storybook/addon-docs/blocks';
import SourceControlled from './examples/controlled.tsx?raw';
import SourceIconOnly from './examples/icon-only.tsx?raw';
import SourceDisabled from './examples/disabled.tsx?raw';
import SourceSizes from './examples/sizes.tsx?raw';
import SourceBasic from './examples/basic.tsx?raw';
import SourceIcon from './examples/icon.tsx?raw';
import SourceOverflow from './examples/overflow.tsx?raw';
import SourceRTL from './examples/rtl.tsx?raw';
import SourceCollapseInactive from './examples/collapse-inactive.tsx?raw';
import * as Stories from './segmented-controller.stories.tsx';

<Meta of={Stories} name="Overview" />

## Installation

```bash
npm install --save @godaddy/antares
```

## Props

<ArgTypes of={Stories.Props} />

## Examples

### Basic

An uncontrolled segmented controller with a default selection. The component manages its own state internally via `defaultValue`.

<Story of={Stories.Basic} inline />
<Source language="tsx" code={SourceBasic} />

### Controlled

Use `value` and `onSelectionChange` to fully control the selected segment from parent state.

<Story of={Stories.Controlled} inline />
<Source language="tsx" code={SourceControlled} />

### Sizes

Three sizes are available: `sm`, `md` (default), and `lg`.

<Story of={Stories.Sizes} inline />
<Source language="tsx" code={SourceSizes} />

### Icon + Text

Segments can include an `Icon` alongside text for added visual context.

<Story of={Stories.Icon} inline />
<Source language="tsx" code={SourceIcon} />

### Icon Only

For compact layouts, segments can contain only icons. Provide an `aria-label` on each item so screen readers can identify the action.

<Story of={Stories.IconOnly} inline />
<Source language="tsx" code={SourceIconOnly} />

### Overflow

When the number of options exceeds the width of the container, the component automatically shows scroll buttons to let users navigate. Users can also scroll horizontally to navigate the options.

<Story of={Stories.Overflow} inline />
<Source language="tsx" code={SourceOverflow} />

### RTL Direction

The segmented controller follows the current **layout direction** (LTR or RTL). By default, that direction is detected automatically from the browser or system settings.

<Story of={Stories.RTL} inline />
<Source language="tsx" code={SourceRTL} />

### Collapse Inactive

Set `collapseInactive` on the controller and pass an `icon` prop to every `SegmentedControllerItem` to render a compact, icon-first variant: only the selected segment shows its label, while the rest collapse to icon-only and surface their label via tooltip on hover or keyboard focus.

The label remains in the accessibility tree (visually hidden) so screen readers still announce each segment's name.

To keep the controller's outer footprint stable across selections — so the surrounding layout doesn't shift when a longer label becomes selected and a shorter one collapses — the controller measures every segment's expanded and collapsed widths at render time and reserves a `min-width` sized for the worst case (longest label selected, every other segment collapsed). Buttons center inside the reserved space.

<Story of={Stories.CollapseInactive} inline />
<Source language="tsx" code={SourceCollapseInactive} />

### Disabled States

Disable the entire group with `isDisabled` on the container, or disable individual segments with `isDisabled` on specific items.

<Story of={Stories.Disabled} inline />
<Source language="tsx" code={SourceDisabled} />

## Customization

### Data Attributes

Components automatically add data attributes for styling different states:

**SegmentedController Container:** `data-disabled`

**SegmentedControllerItem:** `data-selected`, `data-hovered`, `data-pressed`, `data-focus-visible`, `data-disabled`

## Accessibility

### Keyboard Navigation

- **Tab**: Moves focus to/from the segmented controller
- **Arrow Left/Right**: Move to next/previous segment
- **Space/Enter**: Select the focused segment

### ARIA Support

- `role="radiogroup"` on the container
- `role="radio"` on each segment
- `aria-checked` indicates selection state
- `aria-disabled` for disabled segments
- `aria-label` required on the container for screen readers

## Best Practices

### When to Use a Segmented Controller

- When switching between 2-5 related views within the same context
- When all options should be visible at once
- When the selection immediately changes the displayed content

### When Not to Use

- For more than 5 options (use Tabs or Select instead)
- For form input (use RadioGroup instead)
