...
- **Composition**: For custom interiors (leading/trailing controls, icons, nested Select), pass `children` and compose `Label`, `Group`, `Input`, `ControlButton`, and `Select`
...
When `children` is passed, `TextField` is composed. A composed `Group` inherits `size` and `isDisabled` from the field, so set them on `TextField` and leave the interior to the markup.
