...
- **Label, description, error**: Optional label, helper text, and error message
- **Steppers**: Increment and decrement buttons, or `hideStepper` for a plain numeric input
- **Composition**: To customize the interior (for example text steppers instead of `-` and `+` icons), pass `children` and compose `Label`, `Group`, `Input`, and `Button`
...
When `children` is passed, `NumberField` is composed. Use `variant="control"` with `slot="decrement"` and `slot="increment"` on Button so the steppers still change the value. A composed `Group` inherits `size` and `isDisabled` from the field, so set them on `NumberField` and leave the interior to the markup.
