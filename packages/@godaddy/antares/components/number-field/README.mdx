...
- **Composition**: To customize the interior (for example text steppers instead of `-` and `+` icons), pass `children` and compose `Label`, `Group`, `Input`, and `ControlButton`
...
When `children` is passed, `NumberField` is composed. Use `slot="decrement"` and `slot="increment"` on `ControlButton` so the steppers still change the value. A composed `Group` inherits `size` and `isDisabled` from the field, so set them on `NumberField` and leave the interior to the markup.
