...
`CheckboxGroup` is composed: pass `Label`, the `Checkbox` items, `Text slot="description"`, and `FieldError`. The items are wrapped in a `Group` for you - wrap them yourself in a `Group` with your own `direction`/`gap` when you need to lay them out differently. Each `Checkbox` takes label text next to the indicator. Use `orientation` to lay out the items vertically or horizontally.
