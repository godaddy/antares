...
`CheckboxGroup` is composed: pass `Label`, a `Group` holding the `Checkbox` items, `Text slot="description"`, and `FieldError`. Each `Checkbox` takes label text next to the indicator. Use `orientation` to lay out the items vertically or horizontally.
