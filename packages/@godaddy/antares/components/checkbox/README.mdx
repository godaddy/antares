...
`CheckboxGroup` is composed: pass `Label`, a layout (`Flex`) holding the `Checkbox` items, `Text slot="description"`, and `FieldError`. Each `Checkbox` takes label text next to the indicator.

### Data Attributes

Components automatically add data attributes for styling different states:

**CheckboxGroup Container:** `data-invalid`, `data-disabled`, `data-required`, `data-readonly`
...
Set the layout `Flex`'s `direction` and `gap` yourself: `direction="column" gap="md"` stacks
checkboxes vertically, `direction="row" gap="lg"` arranges them in a horizontal row.
