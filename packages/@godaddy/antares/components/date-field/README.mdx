...
DateField values are typed as `CalendarDate` from [`@internationalized/date`][int-date] — a
date-only type with no time and no timezone. Install it alongside `@godaddy/antares`:
...
Locale comes from a host-app `<I18nProvider>`.
