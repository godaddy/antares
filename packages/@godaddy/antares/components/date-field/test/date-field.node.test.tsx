  ...
  describe('#DateField', function dateField() {
    describe('#examples', function examples() {
      it('renders basic example', function basic() {
        const result = renderToString(withTestLocale(<DateFieldBasicExample />));
        expect(result).toMatchSnapshot();
      });

      it('renders with-default-value example', function withDefault() {
        const result = renderToString(withTestLocale(<DateFieldWithDefaultValueExample />));
        expect(result).toContain('2024');
        expect(result).toMatchSnapshot();
      });

      it('renders controlled example', function controlled() {
        const result = renderToString(withTestLocale(<DateFieldControlledExample />));
        expect(result).toMatchSnapshot();
      });

      it('renders with-description example', function withDescription() {
        const result = renderToString(withTestLocale(<DateFieldWithDescriptionExample />));
      ...
      it('renders min-max example', function minMax() {
        const result = renderToString(withTestLocale(<DateFieldMinMaxExample />));
        expect(result).toMatchSnapshot();
      });

      it('renders disabled-required-readonly example', function dro() {
        const result = renderToString(withTestLocale(<DateFieldDisabledRequiredReadOnlyExample />));
