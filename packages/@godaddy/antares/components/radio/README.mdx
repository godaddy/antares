...
`RadioGroup` is composed: pass `Label`, a `Group` holding the `Radio` items, `Text slot="description"`, and `FieldError`. Each `Radio` takes label text next to the indicator. `orientation` sets keyboard-navigation direction, ARIA, and the item layout.
