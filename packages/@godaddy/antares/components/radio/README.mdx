...
`RadioGroup` is composed: pass `Label`, a layout (`Flex`) holding the `Radio` items, `Text slot="description"`, and `FieldError`. Each `Radio` takes label text next to the indicator. `orientation` only sets keyboard-navigation direction and ARIA — match the layout `Flex`'s `direction` to it yourself.
