...
The `FocusLock` component applies data attributes directly to its children without introducing a wrapper element. This example demonstrates multiple children (backdrop and content) along with render props for dynamic styling.
...
responsibility for styling. The component itself has no default styling.
...
The props support render prop functions that receive the focus state. The render
prop function receives a state object with the following properties:
