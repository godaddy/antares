...
describe('@bento/box', function bento() {
  ...
  describe('#defaults', function defaultsmethod() {
    ...
    it('handles ShadowRoot with host', function shadowRoot() {
      const mockHost = {
        ownerDocument: {
          defaultView: { test: 'window' } as any as Window & typeof globalThis
        }
      } as any;
      ...
      const { env } = defaults(mockShadowRoot);
      const win = env.window();

      assume(win).is.a('object');
      assume((win as any).test).equals('window');
    });

    it('handles Document with defaultView', function documentWithDefaultView() {
      const mockDocument = {
        nodeType: 9,
        defaultView: { custom: 'view' } as any
      } as Document;

      const { env } = defaults(mockDocument);
      const doc = env.document();

      assume(doc).equals(mockDocument);
    });

    it('handles Node with ownerDocument', function nodeWithOwnerDocument() {
      const mockNode = {
        ownerDocument: {
          nodeType: 9,
          defaultView: { test: 'window' } as any
        }
      } as Node;

      const { env } = defaults(mockNode);
      const doc = env.document();
      const win = env.window();

      assume(doc).equals(mockNode.ownerDocument);
      assume(win).is.a('object');
    });

    it('handles Window object with document property', function windowObject() {
      const mockDocument = { nodeType: 9 } as Document;
      const mockWindow = {
        document: mockDocument
      } as any as Window;

      const { env } = defaults(mockWindow as any);
      const doc = env.document();

      assume(doc).equals(mockDocument);
    });

    it('falls back to global window and document when root is undefined', function undefinedRoot() {
      const { env } = defaults(undefined);

      // In node environment, these should fall back to global.window/document
      const win = env.window();
      const doc = env.document();

      assume(win).is.a('object');
      assume(doc).is.a('object');
    });

    it('falls back to global window and document when root is null', function nullRoot() {
      const { env } = defaults(null as any);
