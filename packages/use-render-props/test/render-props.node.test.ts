...
describe('@bento/use-render-props', function bento() {
  ...
  describe('useRenderProps', function renderProps() {
    /**
    ...
    function createComponent(name: string, props = {}, slots = {}) {
      ...
      const TestReturn = withSlots(`BentoRenderProps-${name}`, function Component(args) {
        result = useRenderProps(args, { hello: 'world' });
        return React.createElement('div', result[0]);
      });
    ...
    it('is a function', function fn() {
      assume(useRenderProps).is.a('function');
    });
    ...
    describe('apply', function applying() {
      ...
      it('only returns the props that are requested', function requested() {
        const { apply } = createComponent('requested', {
          foo: 'bar'
        });

        const result = apply({ id: 'example' });
